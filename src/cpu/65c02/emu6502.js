...
// let RamBank3=0;
//let RomBank=0;
// let RomBank=BANK_SIZE*3;	//Point to 0xC000-0xFFFF on startup
...
//mem.length=0x40000;
//Variant 3							67mhz
//let mem=new Array(0x40000);
//Variant 4							69mhz (65mhz with banking)
//let mem=new Array(0x40000).fill(0);
//Variant 5
// let mem=new Uint8Array(0x40000);
let running=0;
let cycle_count=0;
let cycle_penalty=0;
let calcAddress=-1;
// let tempCalcBanked=-1;
// let CalcBanked=-1;
let debugflag;
// let ScreenDirty;
let keyBuffer=[];
...
function recordCycle()
{
	...
	let cycleTemp="";
	let cycleBytes=[];
	for (let i=0;i<opLens[bus.read(PC_start)];i++)
	{
		cycleBytes.push(bus.read(PC_start+i));
		cycleTemp+=bus.read(PC_start+i).toString(16).padStart(2,"0")+" ";
	}
...
function memADDRESS()
{
	calcAddress =bus.read(PC++);
	calcAddress+= (0x100*(bus.read(PC++)));
	return bus.read(calcAddress);
}
...
function memIX()
{
	...
	const t0= (bus.read(PC++)+X)&0xFF;
	calcAddress= (bus.read(t0)+(bus.read(t0+1)<<8))&0xFFFF;
	return bus.read(calcAddress);
}
function memIY()
{
	//No banking for pointer since always in ZP
	const t0=bus.read(PC++);
	calcAddress=bus.read(t0)+(bus.read((t0+1)&0xFF)<<8);
	const high_byte=calcAddress&0xFF00;
	calcAddress+=Y;
	if (high_byte!=(calcAddress&0xFF00))
	{
...
function memIZP()
{
	//No banking for pointer since always in ZP
	const t0=bus.read(PC++);
	//calcAddress=bus.read(t0)+(bus.read(t0+1)<<8);
	calcAddress=(bus.read(t0)+(bus.read((t0+1)&0xFF)<<8));
	return bus.read(calcAddress);
}
function memZP()
...
{
	//No banking since always in ZP
	calcAddress=(bus.read(PC++)+X)&0xFF;
	return bus.read(calcAddress);
}
function memZPY()
{
	//No banking since always in ZP
	calcAddress=(bus.read(PC++)+Y)&0xFF;
	return bus.read(calcAddress);
}
...
function subADC(oper)
{
	...
	if (FlagD==1)
	{
		t1= (A&0xF)+(oper&0xF)+FlagC;
		if (t1>9) t1+=6;
		t1+= (A&0xF0)+(oper&0xF0);
		if (t1>=0xA0) t1+= 0x60;
		cycle_count++;
	}
	else t1= A+oper+FlagC;
...
function subRMB(oper)
{
	...
	calcAddress=bus.read(PC++);
	bus.write(calcAddress, bus.read(calcAddress)&oper);
}
...
function subSTA()
	{bus.write(calcAddress, A);}
function subSTX()
	{bus.write(calcAddress, X);}
function subSTY()
	{bus.write(calcAddress, Y);}
function subSTZ()
	{bus.write(calcAddress, 0);}
function subTRB(oper)
{
	if ((A&oper)==0) FlagZ=1;else FlagZ=0;
	bus.write(calcAddress, (A^0xFF)&oper);
}
function subTSB(oper)
{
	if ((A&oper)==0) FlagZ=1;else FlagZ=0;
	bus.write(calcAddress, A|oper);
}
...
function opPHP()													//0x08
{
	bus.write(0x100+SP, FlagC+FlagZ*2+FlagI*4+FlagD*8+FlagB*16+32+FlagV*64+FlagN*128);
	if (SP==0) SP=0xFF;
	else SP--;
	cycle_count+=3;
}
...
function opJSR()													//0x20
{
	bus.write(0x100+SP, (PC+1)>>8);
	if (SP==0) SP=0xFF;else SP--;
	bus.write(0x100+SP, (PC+1)&0xFF);
	if (SP==0) SP=0xFF;else SP--;
	PC= bus.read(PC)+0x100*bus.read(PC+1);
	cycle_count+= 6;
}
...
function opPLP()													//0x28
{
	if(SP==0xFF) SP=0; else SP++;
	let t0= bus.read(0x100+SP);
...
function opJMP_I()													//0x6C
{
	...
	{
		let t0=bus.read(PC)+0x100*bus.read(PC+1);
		PC=bus.read(t0)+0x100*bus.read(t0+1);
	}
	cycle_count+=6;
}
...
{
	let t0=bus.read(PC)+0x100*bus.read(PC+1)+X;
	PC=bus.read(t0)+0x100*bus.read(t0+1);
	cycle_count+=6;
}
...
function opSTA_ADDRESSY()											//0x99
{
	memADDRESSY();
	subSTA();
	//debugMsg("STA_ADDRESSY: " + calcAddress);
