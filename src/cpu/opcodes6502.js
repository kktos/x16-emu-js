...
function rotate(left, logical) {
	let lines = [];
	if (!left) {
		if (!logical) lines.push("let newTopBit = cpu.p.c ? 0x80 : 0x00;");
		...
		if (!logical) lines.push("let newBotBit = cpu.p.c ? 0x01 : 0x00;");
...
function InstructionGen(is65c12) {
	...
	self.readOp = function (addr, reg, spurious) {
		...
		let op;
		if (reg)
			op = reg + " = cpu.bus.cpuRead(" + addr + ");";
		else
			op = "cpu.bus.cpuRead(" + addr + ");";
		if (spurious) op += " // spurious";
		self.append(self.cycle, op, true, addr);
	};
	self.writeOp = function (addr, reg, spurious) {
		self.cycle++;
		let op = "cpu.bus.cpuWrite(" + addr + ", " + reg + ");";
		if (spurious) op += " // spurious";
		self.append(self.cycle, op, true, addr);
	};
	self.zpReadOp = function (addr, reg) {
		self.cycle++;
		self.append(self.cycle, reg + " = cpu.bus.cpuRead(" + addr + ");", false);
	};
	self.zpWriteOp = function (addr, reg) {
		self.cycle++;
		self.append(self.cycle, "cpu.bus.cpuWrite(" + addr + ", " + reg + ");", true);
	};
...
function makeCpuFunctions(cpu, opcodes, is65c12) {
	...
	function getInstruction(opcodeString, needsReg) {
		...
		switch (arg) {
			...
			case "zp,y":
				if (arg === "zp") {
					ig.tick(2);
					ig.append("let addr = cpu.getb() | 0;");
				} else {
					ig.tick(3);
					ig.append("let addr = (cpu.getb() + cpu." + arg[3] + ") & 0xff;");
				}
				if (op.read) {
			...
			case "(,x)":
				...
				ig.append("let zpAddr = (cpu.getb() + cpu.x) & 0xff;");
				ig.append("let lo, hi;");
			...
			case "(),y":
				ig.tick(2);
				ig.append("let zpAddr = cpu.getb() | 0;");
				ig.append("let lo, hi;");
				ig.zpReadOp("zpAddr", "lo");
				ig.zpReadOp("(zpAddr + 1) & 0xff", "hi");
				ig.append("let addr = lo | (hi << 8);");
			...
			case "(abs,x)":
				ig.tick(4);
				ig.append("let addr = (cpu.getw() + cpu.x) | 0;");
				ig.append("let lo, hi;");
				ig.readOp("addr", "lo");
	...
	function Disassemble6502(cpu) {
		...
		this.disassemble = function (addr, plain) {
			...
			if (plain) {
				formatAddr = utils.hexword;
				formatJumpAddr = utils.hexword;
			}
			let opcode = opcodes[cpu.bus.cpuPeek(addr)];
			if (!opcode) {
				return ["???", addr + 1];
			}
			...
			switch (param) {
				...
				case "abs":
					...
					destAddr = cpu.bus.cpuPeek(addr + 1) | (cpu.bus.cpuPeek(addr + 2) << 8);
				...
				case "()":
					destAddr = cpu.bus.cpuPeek(addr + 1);
					destAddr = cpu.bus.cpuPeek(destAddr) | (cpu.bus.cpuPeek(destAddr + 1) << 8);
					return [split[0] + " ($" + utils.hexbyte(cpu.bus.cpuPeek(addr + 1)) + ")" + suffix + " ; $" + utils.hexword(destAddr) + suffix2, addr + 2];
				case "(abs)":
					destAddr = cpu.bus.cpuPeek(addr + 1) | (cpu.bus.cpuPeek(addr + 2) << 8);
					indDest = cpu.bus.cpuPeek(destAddr) | (cpu.bus.cpuPeek(destAddr + 1) << 8);
...
export default {
	cpu6502: (cpu) => makeCpuFunctions(cpu, opcodes6502, false),
	cpu65c12: (cpu) => makeCpuFunctions(cpu, opcodes65c12, false)
};
